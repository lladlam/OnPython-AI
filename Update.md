...
## V0.1 Beta3 (2025年11月29日)

### 新增功能

* **暗色模式** - 支持暗色/浅色主题切换，可设置为根据系统主题自动调整
* **消息按角色着色** - 不同角色的消息使用不同颜色显示（AI、用户、系统）
* **库检查和自动安装** - 程序启动时自动检查必要库和工具，支持一键安装缺失组件
* **主题设置页面** - 在设置界面新增主题配置选项
  * 主题选择（浅色/深色）
  * 自动检测系统主题选项

### 功能改进

* **UI视觉优化** - 改进不同主题下的颜色搭配，提升视觉体验
* **消息显示优化** - 按角色使用不同颜色显示消息，提高可读性
* **库管理优化** - 自动检测并安装缺失的第三方库，提升程序稳定性
* **安装过程优化** - 提供详细的安装进度反馈和错误处理机制
* **配置管理优化** - 主题设置选项集成到配置文件中，确保设置持久化

### 架构优化

* **主题系统模块化** - 将主题相关代码模块化，便于维护和扩展
* **库检查机制** - 实现智能库检查和安装机制
* **UI组件主题适配** - 主要UI组件支持主题切换
* **异步安装处理** - 库安装过程在独立线程中执行，避免阻塞主界面
